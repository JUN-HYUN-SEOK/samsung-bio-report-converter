...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # 데이터 로드 (모든 컬럼을 문자열로 읽어서 앞의 0 보존)
                    ...
                    try:
                        if raw_col in [None, "#N/A"]:
                            ...
                            if raw_col in raw_df.columns:
                                # 원본 값을 그대로 사용 (dtype=str로 읽었기 때문에 이미 문자열)
            ...
            with st.spinner("⚙️ 데이터 변환 중..."):
                ...
                if '신고일자' in final_df.columns:
                    # 날짜 형식으로 변환하여 정렬
                    final_df['신고일자_sort'] = pd.to_datetime(final_df['신고일자'], errors='coerce')
                    final_df = final_df.sort_values('신고일자_sort', ascending=True)
                    final_df = final_df.drop('신고일자_sort', axis=1)
        ...
        with st.expander("🔧 변환 로직 설명", expanded=False):
            ...
            #### 1️⃣ **파일 읽기 & 전처리**
            ```python
            # Excel 파일을 모든 컬럼 문자열로 로드 (앞의 0 보존)
            ...
            - **원본 값 100% 보존**: 모든 데이터를 원본 그대로 유지
            
            ### 🔍 **데이터 품질 보장:**
            - **NULL 값 처리**: 빈 문자열로 통일
            - **앞의 0 보존**: "020" → "020", "00" → "00"
            - **원본 값 유지**: dtype=str로 읽어 모든 값 그대로 보존
            - **RAW 헤더 보존**: 원본 헤더 정보 유지
            """)
